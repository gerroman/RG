...
describeDiagram::usage = "describeDiagram[l, k] make a description for the l-loops and k-legged diagram"

den::usage="den[d, n] = den[{d, n}] = den[{{d, n}}] \[LongDash] denominator 1/d^n
den[{{d1, n1}..}] \[LongDash] product of denominators"

getSector::usage="getSector[p] evaluates sector for the point p"
...
den /: Format[den[{dn:{_,_}}], TraditionalForm] := (
  DisplayForm[FractionBox[1, denBox[dn]]]
);
den /: Format[den[dl:{{_,_}..}], TraditionalForm] := (
  DisplayForm[FractionBox[1, RowBox[denBox /@ dl]]]
);
...
den /: den[l_List] * den[m_List] := den[
  Sort[Join[l, m]] //. {
    {pre___, {a_, x_}, {a_, y_}, post___} :> {pre, {a, x + y}, post}
  }
];
