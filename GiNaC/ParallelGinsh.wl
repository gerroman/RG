...
ginshN[expr_, precision_Integer, batchSize_Integer:100] := Module[{
    prog,
    Gs = DeleteDuplicates[Cases[expr, _G | _H | _MZV | _Li, All]],
    GsN
  },
...
ginshListN[exprList : {(_G | _H | _MZV | _Li) ..}, precision_Integer] := Module[{
    prog,
    GsN,
    toGinsh = Replace[{
      ...
      mzv_MZV :> StringJoin[
        "zeta(",
        ToString[Abs[List @@ mzv], InputForm],
        ",",
        ToString[Sign[List @@ mzv], InputForm],
        ")"
      ],
      li_Li :> ToString[li, InputForm]
    }],
    evalFunction = StringJoin[
      "evalf(",
  ...
  (* use temporary file as stream to  write ginsh program *)
  stream = OpenWrite[BinaryFormat->True];
